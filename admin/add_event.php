    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (in_array($imageFileType, ['jpg', 'jpeg', 'png', 'gif'])) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
                ...
                $stmt = $conn->prepare("INSERT INTO events (event_name, event_type, date, start_time, end_time, location, slots, description, image, created_by, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssissss", $event_name, $event_type, $date, $start_time, $end_time, $location, $slots, $description, $image, $created_by, $status);
...
<body>
    <div class="container">
        <h1>Add New Event</h1>
        <form action="add_event.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()" autocomplete="off">
