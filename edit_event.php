...
if ($result->num_rows == 0) {
    echo "<script>alert('Event not found or you do not have permission to edit this event.'); window.location.href='my_event.php';</script>";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE events SET event_name = ?, event_type = ?, date = ?, start_time = ?, end_time = ?, location = ?, slots = ?, description = ?, image = ? WHERE event_id = ? AND created_by = ?");
    $stmt->bind_param("ssssssissii", $event_name, $event_type, $date, $start_time, $end_time, $location, $slots, $description, $image, $event_id, $_SESSION['user_id']);
